...
define( 'EVENTS_URL', 'https://calendar.mit.edu/api/2/events' );
    ...
    foreach ($jsonIterator as $key => $val) {
	    if(is_array($val)) {  
    		foreach($val as $arraykey => $arrayval) {
    			if (($arraykey == "department_name") && ($arrayval == "MIT Libraries")) {
  					$title =  $val["title"];
					...
					$eventtype = $val["filters"]["event_types"][0]["name"];
					$calendar_id =  $val["id"];
			    	...
			    	__update_post_meta( $post_id, 'event_start_time' , $starttime );
 					...
 					__update_post_meta( $post_id,  'is_event' , 'True' );
 					__update_post_meta( $post_id,  'external_link' , $calendar_url );
